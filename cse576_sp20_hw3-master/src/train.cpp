...
Model neural_net_3(int inputs, int outputs) {
  return {{
              Layer(inputs, 32, TANH),
              Layer(32, 64, TANH),
              Layer(64, 32, TANH),
...
int main(int argc, char **argv) {
  ...
  Model model = neural_net_3(d.train.X.cols,d.train.y.cols);
  ...
  printf("training accuracy: %lf,  test accuracy: %lf\n", model.accuracy(d.train),model.accuracy(d.test));
