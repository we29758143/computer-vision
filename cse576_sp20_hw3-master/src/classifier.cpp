...
Matrix forward_weights(const Layer &l, const Matrix &in) {
  ...
  //     for (int k = 0; k < l.w.cols; k++) {
  //       output(i, j) += l.w(i, k) * in(k, j);
  //     }
  //   }
  // }
  // cout << "fowwwwwwwwward" << endl;
  output = in * l.w;
  // for (int i = 0; i < output.rows; i++) {
  //   for (int j = 0; j < output.cols; j++) {
  ...
  // cout << "PASSSSSSSSSSSSSSSSSS" << output.cols << endl;
...
Matrix Layer::forward(const Matrix &in) {
  ...
  // cout << "forward weights" << endl;
  l.out1 = forward_weights(l, in);     // applying weights
  // cout << "forward activataion" << endl;
...
Matrix backward_w(const Layer &l) {
  ...
  Matrix grad_w;
  // NOT_IMPLEMENTED();
  Matrix transIn = l.in.transpose();
  grad_w = transIn * l.grad_out1;
...
Matrix backward_x(const Layer &l) {
  ...
  Matrix wT = l.w.transpose();
  grad_x = l.grad_out1 * wT;
...
void update_layer(Layer &l, double rate, double momentum, double decay) {
  ...
  // NOT_IMPLEMENTED();
  Matrix last = (-decay * l.w) + l.grad_w;
  Matrix dwt = (momentum * l.v) + last;
  l.v = dwt;
